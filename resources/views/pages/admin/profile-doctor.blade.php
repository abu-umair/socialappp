    ...
    <div class="col-12 col-md-12 col-lg-5">
      <div class="card profile-widget">
        <div class="profile-widget-header ">
          <img alt="image" src="{!!$doctor->foto ? Storage::url($doctor->foto) : url('backend/assets/img/avatar/avatar-1.png')!!}" class="img-thumbnail ">
          <div class="profile-widget-items">
            <div class="profile-widget-item">
              @php
                      ...
                      $created_doctor = \Carbon\Carbon::parse($doctor->created_at);
                      $mydateCust = $created_doctor->toDateString();
                      $myTimeCust = $created_doctor->toTimeString();
                      // $mytimedatetime = $created_doctor->toDateTimeString();
                      ...
                      //$diffHuman = $created_doctor->diffForHumans($past);  // 3 Months ago
                      $timeCust = $convertTimeCust->diffForHumans();
                      $dateCust = $covertDateCust->diffForHumans();
                      //$diffHours = $created_doctor->diffInHours($future);  // 3 
                      //$diffMinutes = $created_doctor->diffInMinutes($nowCust)   // 180

                      // if($created_doctor->diff($nowCust)-->subDays(1)){
                        
                      //   $differenceCust = 'd';
                      // }
                      // elseif($created_doctor->diff($nowCust)->days == 2){
                      //   $differenceCust = 's';
                      // }
                      // else {
                      //   $differenceCust = $dateCust;
                      // }
                      $differenceCust = ($created_doctor->diff($nowCust)->days < 1)
                                    ? $timeCust : $dateCust;
                  @endphp    
              <div class="profile-widget-item-label">Waktu Pendaftaran</div>
              <div class="profile-widget-item-value">{{$differenceCust}}</div>
              {{-- <div class="profile-widget-item-value">{{$doctor->created_at}}</div> --}}
            </div>
            <div class="profile-widget-item">
              <div class="profile-widget-item-label">Kategori</div>
              <div class="profile-widget-item-value">{{$doctor->kategori}}</div>
            </div>
            {{-- <div class="profile-widget-item">
              <div class="profile-widget-item-label">Email</div>
              <div class="profile-widget-item-value">{{$doctor->email}}</div>
            </div>
            <div class="profile-widget-item">
              <div class="profile-widget-item-label">Phone</div>
              <div class="profile-widget-item-value">{{$doctor->phone}}</div>
            </div> --}}
            {{-- <div class="profile-widget-item">
          ...
          {{-- <div class="profile-widget-items">
            <div class="profile-widget-item">
              ...
              <div class="profile-widget-item-value">{{$doctor->lokasi}}</div>
            </div>
          </div> --}}
        ...
        <div class="profile-widget-description">
          <div class="profile-widget-name"><h5 class="d-inline">{{$doctor->name}}</h5> <div class="text-muted d-inline font-weight-normal"><div class="slash"></div> Doctor</div></div>
          <p class="font-weight-normal">
            Transaksi : <b>{{$doctor->transaksi}} Transaksi</b> <br>
            Harga : <b>{{$doctor->harga}}</b> <br>
            Pengalaman : <b>{{$doctor->pengalaman}}</b> <br>
            Lokasi : <b>{{$doctor->lokasi}}</b> <br>
            Status : <b>{{$doctor->status == 0 ? 'Tidak Aktif' : 'Aktif'}}</b> <br>
          </p>
        </div>
          ...
          <div class="card-body">
            <table class="table table-sm">              
              <tbody>
                ...
                <tr class="">
                  <td><p>{{$ongoing->doctor->name}} dibooking dengan customer 
                    {!!'<span class="border-bottom">'.$ongoing->user->name.'</span>'!!} untuk menjadi docter hewannya
