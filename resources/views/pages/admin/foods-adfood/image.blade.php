      ...
      <div class="section-header">
        ...
        <div class="section-header-breadcrumb">
          <div class="section-header-breadcrumb">
              ...
              <div class="breadcrumb-item active"><a >Food</a></div>
            </div>
        </div>
      </div>
          ...
          <div class="card-body">
            ...
            <div class="card">
              <div class="card-header">
                <h4 class=""><code>Images Food</code></h4>
              </div>
              <div class="card-body">
                ...
                {{-- <input type="text" id="stud-id-{{ $item->id }}" value="{{ $item->id }}" /> --}}
                  <div class="d-block">
                    <div class="d-inline">
                      ...
                      <button class="btn btn-info update_food mx-2" type="button" data-id="{{ $item->id }}"> 
                        <i class="fa fa-pencil-alt"></i>
                      </button>

                      <form action="{{ route('foods_image_delete', $item->id) }}"
                        method="POST" class="d-inline" id="dataFood-{{ $item->id }}">
                        @csrf
                        @method('delete')
                      </form>
                      <button class="btn btn-danger" onclick="deleteRowFood( {{ $item->id }} )" > 
                  ...
                  @empty
                    <img src="{!! url('backend/assets/img/avatar/food.png') !!}" class=" mr-2 " width="300">
                  @endforelse
              </div>
            </div>
          </div>
...
<script>
  $(document).on('click', '.update_food', function (e) {
            ...
            $.ajax({
                type: "PUT",
                url: "/admin/update-image-food/" + id,
                data: data,
                dataType: "json",
...
{{-- Sweet Alert delete Food--}}
<script>
  function deleteRowFood(id)
         {
             swal({
               title: 'Are you sure ?',
